...
class ShellAdapter
{
    ...
    public function getParallelSSHPathStatus ($sPath, array $aServers)
    {
        ...
        if (count($aServersToCheck) > 0) {
            ...
            $aParallelResult = $this->parallelize($aServersToCheck, $sPattern, $this->_aConfig['parallelization_max_nb_processes']);
            ...
            $sRsyncPattern='rsync -axz --delete %1$s%2$s--stats -e ssh %3$s %4$s')
    {
        ...
        $aParallelResult = $this->parallelize($aValues, $sRsyncCmd, $this->_aConfig['rsync_max_nb_processes']);
        $aAllResults = array();
        foreach ($aParallelResult as $aServerResult) {
